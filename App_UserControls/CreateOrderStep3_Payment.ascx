...
<script type="text/javascript">
    ...
    var addDicount = {
        init: function(){
            ...
            var disc = localStorage.getItem("discountCode") || localStorage.getItem("discount");

            	if( disc != null ) {
            	    disc = disc.replaceAll("null", "");
                    disc = disc.replaceAll(" ", "&");
            		$(".RegFormDiscount input").val( disc.replaceAll("&", ",") );
            		dicountHelper.init( disc, true );
            	}

            	$(".RegFormDiscount input").change( function () {
            		dicountHelper.init( $(this).val() );
            	});
        }
    }
    function pageLoad() {
...
<div class="cartButtons order-buttons">
    <asp:HyperLink ID="hpBack0" runat="server" NavigateUrl="<%$ Resources: Resource, PageDefaultForUrl %>" Text="PŘIDAT JEŠTĚ DO KOŠÍKU"  CssClass="btn btn-default" />

    <asp:Button ID="btnBackTop" runat="server"
        Text="<%$ Resources: Resource, StepBack %>" CssClass="button btn btn-default"
        OnClick="btnBack_Click" />
        <a href="#" class="button btn btn-basic _validate"> <asp:Literal ID="contTop" runat="server" Text="POKRAČOVAT K ODESLÁNÍ OBJEDNÁVKY"></asp:Literal></a>
    <asp:Button ID="btnContinueTop" runat="server"
        Text="POKRAČOVAT K ODESLÁNÍ OBJEDNÁVKY" class="button btn btn-basic _continue" ValidationGroup="continue"
...
<div class="row">
    ...
    <label class="checkPromo"><input name="checkPromo" type="checkbox" class="more_info" data-open="_promoCode" /> Máte slevový kód?</label>
    <!-- slevovy kod -->
        <div class="orderWayDel order-box _promoCode">
        ...
        <div class="RegFormDiscount">
               <div class="message_discount badCode"><div>Zadali jste špatný slevový kód.</div></div>
               <div class="message_discount platnost"><div>Platnost slevového kódu vypršela.</div></div>
    ...
    <div class="col-md-6">
        <label class="checkPromo"><input name="checkPromo" type="checkbox" class="more_info" data-open="_anotherInfo" /> Potřebujete zadat další údaje?</label>

        <!-- ostatni -->
        <asp:Panel ID="Panel2" runat="server" CssClass="orderReg stats order-box _anotherInfo">
...
<div class="cartButtons order-buttons">
    <asp:HyperLink ID="hpBack" runat="server" NavigateUrl="<%$ Resources: Resource, PageDefaultForUrl %>" Text="PŘIDAT JEŠTĚ DO KOŠÍKU"  CssClass="btn btn-default" />
    <asp:Button ID="btnBackBottom" runat="server"
        Text="<%$ Resources: Resource, StepBack %>" CssClass="button btn btn-default"
        OnClick="btnBack_Click" />
    <a href="#" class="button btn btn-basic _validate"> <asp:Literal ID="cont" runat="server" Text="POKRAČOVAT K ODESLÁNÍ OBJEDNÁVKY"></asp:Literal></a>
    <asp:Button ID="btnContinueBottom" runat="server"
        Text="POKRAČOVAT K ODESLÁNÍ OBJEDNÁVKY" CssClass="button btn btn-basic _continue" ValidationGroup="continue"
        OnClick="btnContinue_Click" />
</div>
</asp:Panel>
